...
class YMTreeGenerator(QMainWindow):
    ...
    def load_settings(self):
        ...
        for i in range(self.font_size_combo.count()):
            if self.font_size_combo.itemData(i) == saved_font_size:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self.compare_versions(latest_version, current_version) > 0:
                    ...
                    if download_asset:
                        # 添加自动更新按钮
                        auto_update_btn = msg.addButton("自动更新", QMessageBox.ActionRole)
                        manual_download_btn = msg.addButton("手动下载", QMessageBox.ActionRole)
                        close_btn = msg.addButton("关闭", QMessageBox.RejectRole)
                        
                        msg.exec_()
                        
                        if msg.clickedButton() == auto_update_btn:
                            self.auto_update(download_asset, latest_version)
                        elif msg.clickedButton() == manual_download_btn:
                            QDesktopServices.openUrl(QUrl(release_data.get("html_url", f"https://github.com/{github_repo}/releases/latest")))
                    else:
                        # 没有找到可下载的文件，只提供手动下载
                        manual_download_btn = msg.addButton("前往下载", QMessageBox.ActionRole)
                        close_btn = msg.addButton("关闭", QMessageBox.RejectRole)
                        
                        msg.exec_()
                        
                        if msg.clickedButton() == manual_download_btn:
                            QDesktopServices.openUrl(QUrl(release_data.get("html_url", f"https://github.com/{github_repo}/releases/latest")))
                else:
                    # 已是最新版本
    ...
    def auto_update(self, download_asset, version):
        """自动更新程序"""
        try:
